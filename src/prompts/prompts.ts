...
- \`${withPrefix('list-assigned-process')}\` — list in-progress workflow runs assigned to the current user (returns \`rs_…\` run state UIDs and their books)
...
0. **\`${withPrefix('list-assigned-process')}\`** — when the user asks about workflows "assigned to me", "my tasks", or "what's waiting on me", start here. It returns the in-progress run states (\`rs_…\`) assigned to the current user with their books; feed a \`bookUid\` + \`runStateUid\` into step 1 to continue an existing run.
